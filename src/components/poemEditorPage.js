...
const EditorPage = () => {
    ...
    const removeLine = (deleteId) => {
        setUserPoem(newPoem => newPoem.filter((line, index) => index !== deleteId));
    };
    ...
    return (
        <div className="relative">
            <form 
                className="scrollbar relative pb-[40px] mb-[50px] p-[40px] w-[850px] h-[700px] rounded-[20px] bg-[#3A141E] overflow-auto overflow-x-hidden">
                ...
                {userPoem.length !== 0 && (
                    <ul 
                        ...
                        <motion.li
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 100 }}
                            transition={{ delay: isGeneratedPoem ? poem.length * 0.3 : 0.3, duration: 0.5 }}
